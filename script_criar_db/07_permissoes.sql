...
-- DOACAO DE ALIMENTOS
ALTER FUNCTION montar_cesta_basica() SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION montar_cesta_basica() FROM PUBLIC;

ALTER FUNCTION formatar_alimentos(alimentos json) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION formatar_alimentos(alimentos json) FROM PUBLIC;

ALTER FUNCTION informar_falecimento_socio(cpf varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION informar_falecimento_socio(cpf varchar) FROM PUBLIC;

ALTER FUNCTION inserir_item_cesta_basica(nome_alimento varchar, qtd int) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION inserir_item_cesta_basica(nome_alimento varchar, qtd int) FROM PUBLIC;

ALTER FUNCTION deletar_item_cesta_basica(nome_alimento varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION deletar_item_cesta_basica(nome_alimento varchar) FROM PUBLIC;

ALTER FUNCTION listar_cesta_basica() SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION listar_cesta_basica() FROM PUBLIC;

ALTER FUNCTION realizar_doacao(cpf_benfeitor varchar, alimentos json, id_doacao int) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION realizar_doacao(cpf_benfeitor varchar, alimentos json, id_doacao int) FROM PUBLIC;

ALTER FUNCTION receber_doacao(cpf_socio varchar, alimentos json, id_recebimento int) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION receber_doacao(cpf_socio varchar, alimentos json, id_recebimento int) FROM PUBLIC;

-- EVENTO
ALTER FUNCTION criar_evento(
nome_evento varchar,
nome_tipo_evento varchar,
valor_arrecacao float,
valor_custo float,
data_inicio date,
data_fim date) SECURITY DEFINER SET search_path = default;
...
ALTER FUNCTION doar_para_evento(nome_evento varchar, cpf_benfeitor varchar, valor_doacao float) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION doar_para_evento(nome_evento varchar, cpf_benfeitor varchar, valor_doacao float) FROM PUBLIC;

ALTER FUNCTION adicionar_custo_evento(nome_evento varchar, valor_custo float) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION adicionar_custo_evento(nome_evento varchar, valor_custo float) FROM PUBLIC;

ALTER FUNCTION atualizar_data_final_evento(nome_evento varchar, data_fim date) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION atualizar_data_final_evento(nome_evento varchar, data_fim date) FROM PUBLIC;

ALTER FUNCTION alocar_voluntario_em_evento(nome_evento varchar, cpf_voluntario varchar, nome_funcao varchar) SECURITY DEFINER SET search_path = default;
...
ALTER FUNCTION remover_voluntario_de_evento(nome_evento varchar, cpf_voluntario varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION remover_voluntario_de_evento(nome_evento varchar, cpf_voluntario varchar) FROM PUBLIC;

ALTER FUNCTION remover_voluntario_de_evento_pela_funcao(
    nome_evento varchar, cpf_voluntario varchar, nome_funcao varchar) SECURITY DEFINER SET search_path = default;
...
ALTER FUNCTION listar_funcoes_voluntario_evento(nome_evento varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION listar_funcoes_voluntario_evento(nome_evento varchar) FROM PUBLIC;

ALTER FUNCTION listar_eventos_por_voluntario(cpf_voluntario varchar, eventos_ativos boolean) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION listar_eventos_por_voluntario(cpf_voluntario varchar, eventos_ativos boolean) FROM PUBLIC;

-- CONSULTA MEDICO

ALTER FUNCTION alocar_medico_em_especialidade(nome_medico varchar, nome_especialidade varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION alocar_medico_em_especialidade(nome_medico varchar, nome_especialidade varchar) FROM PUBLIC;

ALTER FUNCTION remover_especialidade_de_medico(nome_medico varchar, nome_especialidade varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION remover_especialidade_de_medico(nome_medico varchar, nome_especialidade varchar) FROM PUBLIC;

ALTER FUNCTION listar_consultas_medico(_nome_medico varchar, data_consulta date) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION listar_consultas_medico(_nome_medico varchar, data_consulta date) FROM PUBLIC;

ALTER FUNCTION marcar_consulta(
    nome_medico varchar,
    nome_especialidade varchar,
    cpf_socio varchar,
    data date,
    hora time) SECURITY DEFINER SET search_path = default;
...
ALTER FUNCTION buscar_chave_valor(nome_tabela varchar, chave varchar, valor varchar) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION buscar_chave_valor(nome_tabela varchar, chave varchar, valor varchar) FROM PUBLIC;

ALTER FUNCTION atualizar_generico(nome_tabela varchar, chave varchar, valor varchar,  campos json, forbidden_fields text[]) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION
    atualizar_generico(nome_tabela varchar, chave varchar, valor varchar,  campos json, forbidden_fields text[])
FROM PUBLIC;

ALTER FUNCTION cadastrar_generico(nome_tabela varchar, campos json, forbidden_fields text[]) SECURITY DEFINER SET search_path = default;
REVOKE ALL ON FUNCTION cadastrar_generico(nome_tabela varchar, campos json, forbidden_fields text[]) FROM PUBLIC;

ALTER FUNCTION deletar_generico(nome_tabela varchar, chave varchar, valor varchar) SECURITY DEFINER SET search_path = default;
...
GRANT EXECUTE ON FUNCTION doar_para_evento(nome_evento varchar, cpf_benfeitor varchar, valor_doacao float) TO funcionario;
GRANT EXECUTE ON FUNCTION adicionar_custo_evento(nome_evento varchar, valor_custo float)  TO funcionario;
GRANT EXECUTE ON FUNCTION atualizar_data_final_evento(nome_evento varchar, data_fim date)  TO funcionario;
